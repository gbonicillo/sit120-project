<template>
    <general-contents-container page-title="Pending Orders">
        <h2 class="text-muted">
...
export default {
    ...
    async asyncData ({ $axios, params, redirect, error }) {
        try {
            ...
            const ordersResult = await $axios.$get(`/api/shops/${result.id}/orders?search=PD`);
            return {
    ...
    methods: {
        ...
        updatePage () {
            this.fetchPage(`/api/shops/${this.karenderyaId}/orders?search=PD`);
        }
    }
};
</script>
