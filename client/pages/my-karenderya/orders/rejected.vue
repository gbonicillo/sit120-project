<template>
    <general-contents-container page-title="Rejected Orders">
        <h2 class="text-muted">
...
export default {
    ...
    async asyncData ({ $axios, params, redirect, error }) {
        try {
            ...
            const ordersResult = await $axios.$get(`/api/shops/${result.id}/orders?search=RJ`);
            return {
